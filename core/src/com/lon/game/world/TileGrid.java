...
public class TileGrid implements Drawable {
    ...
    public TileGrid(int width, int height, TileFactory tileFactory, TypeStrategy typeStrategy) {
        this.width = width;
        this.height = height;
    ...
    private List<List<Tile>> initGrid() {
        ...
        for (int i = 0; i < height; i++) {
            ...
            for (int j = 0; j < width; j++) {
                row.add(tileFactory.createTile(new Vector2(j, i), typeStrategy.getType(i, j)));
            }
            map.add(row);
        }
